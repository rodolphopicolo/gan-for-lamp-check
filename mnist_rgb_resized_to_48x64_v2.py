...
def make_generator_model():
    ...
    model.add(layers.Conv3DTranspose(64, (5, 5, 3), strides=(2, 2, 1), padding='same', use_bias=False))

    print('Gen output shape 8:', model.output_shape)

    assert model.output_shape == (None, 24, 32, 3, 64)
    ...
    model.add(layers.Conv3DTranspose(1, (5, 5, 3), strides=(2, 2, 1), padding='same', use_bias=False, activation='tanh'))
...
def make_discriminator_model():
    model = tf.keras.Sequential()
    model.add(layers.Conv3D(64, (5, 5, 1), strides=(2, 2, 1), padding='same', input_shape=[48, 64, 3, 1]))
    model.add(layers.LeakyReLU())
    model.add(layers.Dropout(0.3))

    model.add(layers.Conv3D(128, (5, 5, 1), strides=(2, 2, 1), padding='same'))
    model.add(layers.LeakyReLU())
    model.add(layers.Dropout(0.3))
...
checkpoint_dir = './training_checkpoints/mnist_rgb_resized_48x64_training_checkpoints_v2'
...
def generate_and_save_images(model, epoch, test_input):
  ...
  for i in range(predictions.shape[0]):
    image_name = './images/mnist_rgb_resized_48x64_images_v2/image_at_epoch_{:04d}.png'.format(epoch)
    image = images[i]
