...
train_images_48x48 = np.zeros((train_images.shape[0], 48, 48))
train_images_48x48[:, :, :] = 127

train_images_48x48[:, :train_images.shape[1], :train_images.shape[2]] = train_images

train_images = np.zeros((train_images.shape[0], 48, 48))

train_images = train_images_48x48
...
def make_generator_model():
    ...
    model.add(layers.Dense(12*12*3*256, use_bias=False, input_shape=(NOISE_DIM,)))
    ...
    model.add(layers.Reshape((12, 12, 3, 256)))

    print('Gen output shape 4:', model.output_shape)

    assert model.output_shape == (None, 12, 12, 3, 256)  # Note: None is the batch size
    ...
    assert model.output_shape == (None, 12, 12, 3, 128)
    ...
    assert model.output_shape == (None, 24, 24, 6, 64)
    ...
    assert model.output_shape == (None, 48, 48, 3, 1)
...
def make_discriminator_model():
    model = tf.keras.Sequential()
    model.add(layers.Conv3D(64, (5, 5, 3), strides=(2, 2, 2), padding='same', input_shape=[48, 48, 3, 1]))
    model.add(layers.LeakyReLU())
...
checkpoint_dir = './training_checkpoints/mnist_rgb_resized_48x48_training_checkpoints'
...
def generate_and_save_images(model, epoch, test_input):
  ...
  for i in range(predictions.shape[0]):
    image_name = './images/mnist_rgb_resized_48x48_images/image_at_epoch_{:04d}.png'.format(epoch)
    image = images[i]
    plt.axis("off")
    plt.imshow(image)
